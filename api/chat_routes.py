...
class ChatAPI:
    ...
    def register_routes(self, bp):
        ...
        def chat():
            ...
            # جستجو در دانش
            results = self.brain.search(question, user_id=user_id)
        ...
        def chat_history():
            ...
            # دریافت از کش
            cache_key = self.cache.make_key('history', user_id, limit)
            cached = self.cache.get(cache_key)
            if cached:
                return jsonify(cached)
                
            # از پروفایل کاربر
            profile = self.learning.get_user_profile(user_id)
        ...
        def get_suggestions():
            ...
            # توصیه‌ها
            recommendations = self.learning.get_recommendations(user_id)
            
            # موضوعات داغ
            trending = self.learning.get_trending_topics()
